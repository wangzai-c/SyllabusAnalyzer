...
library(stringr)
library(stopwords)
library(magrittr) # needs to be run every time you start R and want to use %>%,The pipe operator %>% was introduced to "decrease development time and to improve readability and maintainability of code."
library(textstem)
library(tm)
library(tokenizers)
library(expss)
...
#read file
cased <- read.csv(file.path("syllabus_analyzer\data\cased.csv"),stringsAsFactors = F)
syllabus_f <- read_file("syllabus_analyzer\data\applied_project.txt")
...
word_tf <- cbind(word_tf,tibble(syllabus_tf))
